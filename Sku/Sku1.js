window.addEventListener('load', function() {
    ...
    function SHITIDONGHUA(bjs, jls, callback) {
        var obj = document.querySelector(bjs);
        clearInterval(obj.time);
        obj.time = setInterval(function() {
            var jl = (jls - obj.scrollTop) / 10;
            jl = jl > 0 ? Math.ceil(jl) : Math.floor(jl);
            if (obj.scrollTop == jls) {
                clearInterval(obj.time);
                if (callback) {
                    callback();
                }
            }
            obj.scroll(0, obj.scrollTop + jl);
        }, 15);
    }
    ...
    function SHITIDONGHUA2(bjs, jls, callback) {
        ...
        clearInterval(obj.time);
        obj.time = setInterval(function() {
            var jl = (jlss - obj.scrollTop) / 10;
            jl = jl > 0 ? Math.ceil(jl) : Math.floor(jl);
            if (Math.round(obj.scrollTop) == jlss) {
                clearInterval(obj.time);
                if (callback) {
                    callback();
                }
    ...
    function music_cd_ks() {
        ...
        for (var i = 0; i < i_music_s_ks_tb.length; i++) {
            ...
            i_music_s_ks_tb[i].addEventListener('click', function(e) {
                ...
                this.parentNode.style.backgroundColor = RGB_zhq(localStorage.zi_ti_click_color, 0.1);
                // 播放音乐
                console.clear();
    ...
    function music_glxs() {
        ...
        music_s[music_glxs].style.backgroundColor = RGB_zhq(localStorage.zi_ti_click_color, 0.1);
    }
    ...
    function music_kss() {
        if (yy_ks == 0) {
            ...
            var music_cd = JSON.parse(localStorage.music_cd);
            var music_glxs = music_cd[1].indexOf(music_bfq.src);
            music_boyyom_gm.innerText = music_cd[0][music_glxs];
            ...
            music_bfq.play().then(function() { // 播放成功
                ...
                // 删除高亮
                var music_s = document.querySelectorAll('.music_s');
                music_s[music_glxs].querySelector('.i_music_s_cs_tp').style.opacity = '1';
                // 下一首死机保护
                music_max_sj++;
    ...
    music_bottom_hk2.addEventListener('input', function(e) {
        ...
        music_boyyom_sssj.innerText = Math.floor(((this.value / 100) * music_bfq.duration) / 60) + ':' + (Math.floor(((this.value / 100) * music_bfq.duration) % 60) < 10 ? '0' + Math.floor(((this.value / 100) * music_bfq.duration) % 60) : Math.floor(((this.value / 100) * music_bfq.duration) % 60)) + ' /';
    });
    music_bottom_hk2.addEventListener('mousedown', function(e) {
        music_bfq.removeEventListener('timeupdate', music_ssjc);
    });
    music_bottom_hk2.addEventListener('mouseup', function(e) {
        music_bfq.currentTime = (e.offsetX / this.offsetWidth) * music_bfq.duration;
        music_bfq.addEventListener('timeupdate', music_ssjc);
        music_kss();
    });
    ...
    sy_djs_txl_jsq = setInterval(function() {
        ...
        if (sy_djs_r_s_one !== undefined) {
            ...
            if (nrmaxs0_nr.scrollTop < 10 && sy_djs_r_s_1 == '时间已到<br>超过三天自动删除') {
                sy_djs_txl.style.display = 'block';
                sy_djs_txl.innerHTML = '“' + sy_djs_r_t_1.innerHTML + '”时间已到 ↓';
            } else {
